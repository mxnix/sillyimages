...
- **Размер**: `auto`, `256x256`, `512x512`, `1024x1024`, `1536x1024`, `1024x1536`, `2048x2048`, `4096x4096` (для OpenAI)
...
- Для моделей, которые провайдер отдает не через `/v1/images/generations`, расширение автоматически переключается на совместимый маршрут по `GET /v1/models/{model}`: например на `/v1/chat/completions` или `/v1/responses`
